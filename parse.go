...
func main() {
	...
	if err != nil {
		fmt.Printf("unable to read bytes from %s\n", filepath, err)
		os.Exit(1)
	}
	info, err := compiler.ReadInfoFromBytes(filepath, bytes)
	if err != nil {
		fmt.Printf("unable to read info from %s\n", filepath, err)
		os.Exit(1)
	}

	document, err := openapi_v3.NewDocument(info, compiler.NewContext("$root", nil))
	if err != nil {
		fmt.Printf("unable to parse document %s\n", filepath, err)
		os.Exit(1)
	}

	data, err := json.Marshal(document)
	if err != nil {
		fmt.Printf("unable to marshal document to JSON %s\n", filepath, err)
		os.Exit(1)
	}
	fmt.Printf("%s\n", data)
	os.Exit(0)
